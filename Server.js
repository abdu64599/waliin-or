...
const JWT_SECRET =
process.env.JWT_SECRET || "waliin-or-change-this-secret";
...
await client.query(`
  ALTER TABLE users
  ADD COLUMN IF NOT EXISTS status VARCHAR(30) DEFAULT 'offline'
`);
...
"SELECT id, username, email, bio, avatar, status, last_seen, created_at FROM users WHERE username=$1",
...
await pool.query(
"INSERT INTO notifications (to_user, from_user, message) VALUES ($1,$2,$3)",
[
toUser,
fromUser || null,
message
...
if (!result.rows.length) {
  return res.status(401).json({
    error:
      "Username/email ykn password sirrii miti."
  });
}
...
app.get("/api/notifications/:username", authMiddleware, async (req, res) => {
try {
const username = req.user.username;
...
try {
await pool.query(
"UPDATE notifications SET is_read=true WHERE to_user=$1",
[req.user.username]
);
...
app.get("/api/notifications/unread-count/:username", authMiddleware, async (req, res) => {
try {
const result = await pool.query(
"SELECT COUNT(*)::int AS count FROM notifications WHERE to_user=$1 AND is_read=false",
[req.user.username]
);
...
name: String(name || "Waliin OR Club").slice(0, 100),
...
if (socket.rooms.has("club:${club.id}")) {
return club;
}
}
...
const sender = socket.username;
  ...
  if (blocked.rows.length) {
    socket.emit("messageError", {
      error:
        "User tokko keessan keessaa tokko block godheera."
    });
...
cleanUsername(data?.username);
  ...
  const name =
    String(data?.name || "Waliin OR Club");
...
const username = socket.username;
  ...
  const ownerSocket =
    getUserSocket(club.owner);

  if (ownerSocket) {
    io.to(ownerSocket).emit(
      "clubUpdated",
      club
    );
...
const owner = socket.username;
  ...
  club.audience =
    club.audience.filter(
      (u) => u !== target
    );

  club.seatRequests =
    club.seatRequests.filter(
      (u) => u !== target
    );
...
const owner = socket.username;
  ...
  if (targetSocket) {
    io.to(targetSocket).emit(
      ...
      {
        message:
          "Owner microphone kee mute godheera."
      }
    );
  }
...
const owner = socket.username;
  ...
  if (targetSocket) {
    ...
    const targetSocketObj =
      io.sockets.sockets.get(
        targetSocket
      );

    if (targetSocketObj) {
      targetSocketObj.leave(
        `club:${club.id}`
      );
    }
  }
